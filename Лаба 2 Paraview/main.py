...
# РАБОТАЕТ ДОВОЛЬНО ДОЛГО, ЧТОБЫ РАБОТАЛО БЫСТРЕЕ, МОЖНО УМЕНЬШИТЬ РАЗМЕР СЕТКИ
# НО ТОГДА ПРОПАДАЮТ ПОЛОСЫ
...
class CalcMesh:
    ...
    def __init__(self, nodes_coords, tetrs_points):
        ...
        # СКАЛЯРНАЯ ВЕЛИЧИНА - СИНУС, СОЗДАЮЩИЙ ПОЛОСЫ ПО ОСИ X
        # Модельная скалярная величина распределена как-то вот так
        self.smth = [math.sin(math.pi * abs(self.nodes[0, i])) * 100 for i in
                     range(len(nodes_coords) // 3)]
        ...
        # РАСПРЕДЕЛЕНИЕ СКОРОСТЕЙ ДЛЯ ПОВОРОТА ГОЛОВЫ ВОКРУГ ОСИ Z
        for i in range(len(nodes_coords) // 3):
            if nodes_coords[i * 3 + 1] < -0.44:
                x = abs(nodes_coords[i * 3])
                y = abs((nodes_coords[i * 3 + 1]) + 0.44)       # ПАРАМЕТРЫ ПОДБИРАЛ ПРИМЕРНО, ПОЭТОМУ ДВИЖЕНИЕ НЕ
                alpha = math.atan(y / x)                        # СЛИШКОМ ТОЧНОЕ
                V0 = 1 * (x ** 2 + y ** 2) ** 0.5               # V = wR
                self.velocity[0][i] = -V0 * math.sin(alpha)     # V_y = -V*sin(alpha)
                self.velocity[1][i] = V0 * math.cos(alpha)      # V_x =  V*cos(alpha)
    ...
    def snapshot(self, snap_number):
        ...
        writer.SetFileName("horse_step" + str(snap_number) + ".vtu")
        writer.Write()
